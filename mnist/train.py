...
if __name__ == "__main__":
    ...
    (X, y), (X_test, y_test) = load_mnist_dataset(params)
    print("\n### Source Dataset:")
    ...
    if selector_name == "random":
        selector = BaselineSelector(X, y, train_size=selector_init_size, seed=seed)
    elif selector_name == "lc":
        selector = LeastConfidenceSelector(
            X, y, train_size=selector_init_size, seed=seed
        )
    elif selector_name == "sm":
        selector = SmallestMarginSelector(
            X, y, train_size=selector_init_size, seed=seed
        )
    ...
    for stage in params["train"]["stages"]:
        for i in range(stage["count"]):
            if first_run:
                ...
                X_train, X_reserve, y_train, y_reserve = selector.select(
                    model, size=stage["size"]
                )
                selector.print()

            X_valid, y_valid = X_reserve, y_reserve
            if len(X_valid) > 20000:
